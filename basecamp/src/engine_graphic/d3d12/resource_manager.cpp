...
namespace D3D12 {
    std::shared_ptr<Buffer> Resource_manager::create_static_buffer(const string& name, uint32_t byte_size, const void* init_data)
    {
        ...
        if (init_data) {
            ...
            auto&& barrier = CD3DX12_RESOURCE_BARRIER::Transition(buffer->m_buffer.Get(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_COPY_DEST);
            ...
            barrier = CD3DX12_RESOURCE_BARRIER::Transition(buffer->m_buffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_GENERIC_READ);
            command_list->ResourceBarrier(1, &barrier);
    ...
    std::shared_ptr<Buffer> Resource_manager::create_texture(const string& name, const CD3DX12_RESOURCE_DESC& info, const CD3DX12_CLEAR_VALUE* clear_val,
        const TextureData* init_data, D3D12_RESOURCE_STATES init_state)
    {
        ...
        if (init_data) {
            ...
            // will copy the CPU memory into the intermediate upload heap.  Then, using ID3D12CommandList::CopySubresourceRegion,
            // the intermediate upload heap data will be copied to mBuffer.
            auto&& barrier = CD3DX12_RESOURCE_BARRIER::Transition(buffer->m_buffer.Get(), init_state, D3D12_RESOURCE_STATE_COPY_DEST);
            command_list->ResourceBarrier(1, &barrier);
            ...
            barrier = CD3DX12_RESOURCE_BARRIER::Transition(buffer->m_buffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, init_state);
            command_list->ResourceBarrier(1, &barrier);
    ...
    std::weak_ptr<Buffer> Resource_manager::create_buffer(
        const string& name, const Buffer_request& info, const void* init_data, D3D12_RESOURCE_STATES init_state)
    {
    ...
    std::shared_ptr<Buffer> Resource_manager::create_acceleration_structure(const string& name, const Mesh_buffer& mesh_buffer, bool persistent)
    {
        ...
        std::weak_ptr<Buffer> tlas_buffer, blas_buffer;
        {
            ...
            tlas_resource_req.desc =
                CD3DX12_RESOURCE_DESC::Buffer(top_level_prebuild_info.ResultDataMaxSizeInBytes, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
        ...
        auto&& barrier = CD3DX12_RESOURCE_BARRIER::UAV(blas_buffer.lock()->m_buffer.Get());
        command_list->ResourceBarrier(1, &barrier);
