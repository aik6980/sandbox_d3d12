...
namespace D3D12 {
	...
	class Device {
	  public:
		...
		/// low level
		ID3D12Device5* d3d_device()
		{
			return m_device.Get();
		}
		Command_list& commmand_list()
		{
			return m_commandList;
		}
		FrameResource& frame_resource()
		{
			return *m_frame_resource_list[m_curr_frame_resource_index];
		}

		ID3D12Resource& curr_backbuffer() const
		{
			return *m_swap_chain_buffer[m_curr_swap_chain_buffer].Get();
		}
	  ...
	  private:
		...
		// imgui
		const uint32_t m_num_imgui_srv_descriptor = 1;
		uint32_t	   m_imgui_srv_descriptor_id  = 0;
		...
		ComPtr<ID3D12DescriptorHeap> m_rtv_heap_resource;
		ComPtr<ID3D12DescriptorHeap> m_dsv_heap_resource;
		ComPtr<ID3D12DescriptorHeap> m_srv_heap_resource;
		ComPtr<ID3D12DescriptorHeap> m_sampler_heap_resource;

		Descriptor_heap m_rtv_heap;
		Descriptor_heap m_dsv_heap;
		Descriptor_heap m_srv_heap;
		Descriptor_heap m_static_srv_heap;
